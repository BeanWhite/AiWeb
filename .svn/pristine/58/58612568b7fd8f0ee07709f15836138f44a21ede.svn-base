function toJump(id, url) {
	switch (id) {
		...
		case 38:
			window.close();
			window.location.href = '../html/doc_foreign.html?' + USER_ID + '&' + '总体衰退量表&38';
		case 39:
			window.close();
			window.location.href = '../html/doc_foreign.html?' + USER_ID + '&' + '幸福指数自评量表&39';
		case 40:
			window.close();
			window.location.href = '../html/doc_foreign.html?' + USER_ID + '&' + '社会活动功能调查量表&40';
		case 41:
			window.close();
			window.location.href = '../html/doc_foreign.html?' + USER_ID + '&' + '康奈尔痴呆抑郁量表&41';
		case 42:
			window.close();
			window.location.href = '../html/doc_foreign.html?' + USER_ID + '&' + '简易智力检测量表AMTS&42';
		default:
			console.log('错误指向')
			break;
	}
